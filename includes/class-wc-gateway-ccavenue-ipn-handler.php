...
class WC_Gateway_CCAvenue_IPN_Handler extends WC_Gateway_CCAvenue_Response {
	...
	public function valid_response( $posted ) {
		...
		if(isset($posted['encResp']))
		{
			...
			if($order_id != ''){
				try{
					global $woocommerce;
					$order = new WC_Order($_SESSION['ccavenue_order']['order_id']);
	...
	protected function payment_status_success( $order, $posted ) {
		...
		$order_sess = new WC_Order($_SESSION['ccavenue_order']['order_id']);
		// echo $posted['order_id'].'=='.$_SESSION['ccavenue_order']['order_id'].'++'.number_format($posted['amount'], 2).'=='.$_SESSION['ccavenue_order']['amount'].'++'.$posted['currency'].'=='.$_SESSION['ccavenue_order']['currency'];exit;
		if($posted['order_id'] == $_SESSION['ccavenue_order']['order_id'] && number_format($posted['amount'], 2) == $_SESSION['ccavenue_order']['amount']  && $posted['currency'] == $_SESSION['ccavenue_order']['currency']){
			...
			$order_sess->update_status( 'on-hold', sprintf( __( 'Validation error:Security Error. Illegal access detected', 'ccave' ), $_SESSION['ccavenue_order']['order_id']) );
