...
function wc_gateway_ccavenue_init() {
	...
	class WC_Gateway_CCAvenue extends WC_Payment_Gateway {
		...
		public function __construct() {
			...
			$this->icon         	  = ' https://www.ccavenue.com/images_shoppingcart/ccavenue_pay_options.gif "style="max-width:92%;max-height:100px;float:unset;margin-left:30px;display:block;';
		...
		public function admin_options() {
			...
			echo'<a href ="https://www.ccavenue.com" target="_blank"><img src="https://www.ccavenue.com/images_shoppingcart/ccavenue_logo_india.png" alt="CCAvenue Logo" title="CCAvenue MCPG" /></a></p>';
			echo '<a class="support" style=" font-size:16px;font-family:Verdana, Geneva, sans-serif; color:#09F;" href="mailto:shoppingcart@ccavenue.com?subject=India%20Shopping%20Cart%20-%20Wordpress%205.1%20WooCommerce%203.5" alt="Contact Support" title="Contact Support"> '._('Contact Support').'</a>';
		...
		public function get_request_url( $order, $sandbox ) {
			if ( $sandbox == true ) {
				return 'https://test.ccavenue.com/transaction.do?command=initiateTransaction';	
			} else {
		...
		/**
			...
			$_SESSION['ccavenue_order']['order_id'] = $order_id;
			$_SESSION['ccavenue_order']['amount'] = ( WC()->version < '3.5.0' ) ? $order->order_total : $order->get_total();
			$_SESSION['ccavenue_order']['currency'] = get_woocommerce_currency();  
	  ...
	  public function generate_ccavenue_form($order_id){
			...
			$ccavenue_args = array(
			        ...
			        'amount'           => ( WC()->version < '3.5.0' ) ? $order->order_total : $order->get_total(),
			        'order_id'         => $order_id,
			        'redirect_url'     => $this->notify_url,
			        'cancel_url'       => $this->notify_url,
			        'billing_name'     => (( WC()->version < '3.5.0' ) ? $order->billing_first_name : $order->get_billing_first_name()) .' '. (( WC()->version < '3.5.0' ) ? $order->billing_last_name : $order->get_billing_last_name()),
			        'billing_address'  => trim(( WC()->version < '3.5.0' ) ? $order->billing_address_1 : $order->get_billing_address_1(), ','),
			        'billing_country'  => wc()->countries->countries [( WC()->version < '3.5.0' ) ? $order->billing_country : $order->get_billing_country()],
			        'billing_state'    => ( WC()->version < '3.5.0' ) ? $order->billing_state : $order->get_billing_state(),
			        'billing_city'     => ( WC()->version < '3.5.0' ) ? $order->billing_city : $order->get_billing_city(),
			        'billing_zip'      => ( WC()->version < '3.5.0' ) ? $order->billing_postcode : $order->get_billing_postcode(),
			        'billing_tel'      => ( WC()->version < '3.5.0' ) ? $order->billing_phone : $order->get_billing_phone(),
			        'billing_email'    => ( WC()->version < '3.5.0' ) ? $order->billing_email : $order->get_billing_email(),
			        'delivery_name'    => (( WC()->version < '3.5.0' ) ? $order->shipping_first_name : $order->get_shipping_first_name()) .' '. (( WC()->version < '3.5.0' ) ? $order->shipping_last_name : $order->get_shipping_last_name()),
			        'delivery_address' => ( WC()->version < '3.5.0' ) ? $order->shipping_address_1 : $order->get_shipping_address_1(),
			        'delivery_country' => ( WC()->version < '3.5.0' ) ? $order->shipping_country : $order->get_shipping_country(),
			        'delivery_state'   => ( WC()->version < '3.5.0' ) ? $order->shipping_state : $order->get_shipping_state(),
			        'delivery_tel'     => '',
			        'delivery_city'    => ( WC()->version < '3.5.0' ) ? $order->shipping_city : $order->get_shipping_city(),
			        'delivery_zip'     => ( WC()->version < '3.5.0' ) ? $order->shipping_postcode : $order->get_shipping_postcode(),
			        'language'         => 'EN',
